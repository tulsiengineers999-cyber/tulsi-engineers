...
export default function NewJobPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setFormError(null);
    try {
      const job = await api.post<{ id: string }>("/api/jobs", values);
  ...
  return (
    <>
      <PageHeader
        ...
        description="Link a customer, site and (optionally) equipment to the work that needs to be carried out."
        crumbs={[{ label: "Service Jobs", href: "/jobs" }, { label: "New" }]}
      />
        ...
        <Card title="Where">
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="Customer" required error={errors.customerId}>
              <Select value={values.customerId} onChange={setCustomer} required autoFocus>
