...
export async function GET(req: NextRequest) {
  try {
    ...
    const and: Prisma.WhatsappLogWhereInput[] = [];
    ...
    const where: Prisma.WhatsappLogWhereInput = and.length ? { AND: and } : {};

    const [items, total] = await Promise.all([
      prisma.whatsappLog.findMany({ where, skip, take, orderBy: { createdAt: "desc" } }),
